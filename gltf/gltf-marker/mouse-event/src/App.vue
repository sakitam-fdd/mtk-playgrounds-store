...
<script setup lang="ts">
  ...
  gltfMarker.on("click", () => {
    document.mapRef.value.innerHTML = "click事件";
  });

  gltfMarker.on("mouseenter", () => {
    gltfMarker.setUniform("polygonFill", [0.0, 0.8, 0.0, 1.0]);
    document.mapRef.value.innerHTML = "mouseenter事件";
  });

  gltfMarker.on("mouseout", () => {
    map.resetCursor();
    gltfMarker.setUniform("polygonFill", [1, 1, 1, 1.0]);
    document.mapRef.value.innerHTML = "mouseout事件";
  });

  gltfMarker.on("dblclick", () => {
    map.resetCursor();
    gltfMarker.setUniform("polygonFill", [1, 1, 1, 1.0]);
    document.mapRef.value.innerHTML = "dblclick事件";
  });
