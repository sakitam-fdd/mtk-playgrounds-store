...
<script setup lang="ts">
  ...
  function initMap() {
    ...
    const tileLayer = new TileLayer('base', {
      ...
      subdomains: ['a', 'b', 'c', 'd'],
      attribution: '&copy; <a href="https://www.supermap.com/cn/">supermap</a> contributors',
      tileSystem: [1, -1].concat(parmas.origin as [number, number]) as [number, number, number, number], // tile system
    });
    ...
    tileLayer.getTileUrl = function (x, y, z) {
      ...
      const resolution = Math.max(Math.abs(ne.x - sw.x) / tileSize, Math.abs(ne.y - sw.y) / tileSize);
