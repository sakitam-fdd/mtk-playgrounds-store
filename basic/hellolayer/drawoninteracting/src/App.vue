...
<script setup lang="ts">
  ...
  import { Map, TileLayer, Layer, renderer, Coordinate } from 'maptalks';
  ...
  function initMap() {
    ...
    class HelloLayer extends Layer {
      data: any;
      // 构造函数
      constructor(id, data?: any, options?: any) {
        super(id, options);
        this.data = data;
      }
    ...
    class HelloLayerRenderer extends renderer.CanvasRenderer {
      checkResources() {
      ...
      _drawData(data, color) {
        ...
        data.forEach((d) => {
          ...
          const point = map.coordinateToContainerPoint(new Coordinate(d.coord));
